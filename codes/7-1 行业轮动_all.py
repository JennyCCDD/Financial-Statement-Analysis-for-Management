...
class Industry():
    ...
    def every_month(self):
        ...
        for i,currentDate in enumerate(tqdm(self.tradingDateList[:-2])):
            ...
            # 输出行业因子中位数，用于输出dataframe画图
            self.industry_factor =self.dataFrame.groupby(by='Industry', as_index=False)['factor'].median()
...
if __name__ == "__main__":
    ...
    for factor_i in factor_list:
        main_fun = Industry(para, factor_i)
        des = main_fun.DES()
        ...
        # print(test_nav_abs)
    # In[]
        in_factor.to_csv(para.result_path + '_' + factor_i + '_' + para.weightMethod +
        ...
        # pd.DataFrame(corrList).to_csv(para.result_path + '_' + factor_i + '_' + para.weightMethod +
